...
void msdRadixSort(vector<string>& values, vector<string>& buffer, int left, int right, int depth) {
    if (right - left <= 1) {
        return;
    }
    ...
    for (int key = 1; key <= alphabet; ++key) {
        ...
        if (to - from > 1) {
            msdRadixSort(values, buffer, from, to, depth + 1);
        }
    }
}
...
int main() {
    ...
    msdRadixSort(values, buffer, 0, n, 0);
